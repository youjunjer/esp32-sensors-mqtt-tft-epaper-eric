...
const char* MQTT_DATA_TOPIC = "eric/class305/data";
...
void partialRegion(int logicalX0,int logicalX1,int logicalY0,int logicalY1){
  ...
  int rows = y1 - y0;
  // The logical number band spans almost the full 296-pixel width. After
  // coordinate rotation it becomes 276 native rows by 4 bytes per row.
  uint8_t region[4 * EPD_HEIGHT];
  if(rowBytes > 4 || rows > EPD_HEIGHT) return;
...
void setLedFromMessage(const String& message, int pin, const char* name){
  if(message.indexOf("off") >= 0){
    digitalWrite(pin, LOW);
    Serial.printf("MQTT %s OFF\n", name);
  }else if(message.indexOf("on") >= 0){
    digitalWrite(pin, HIGH);
    Serial.printf("MQTT %s ON\n", name);
  }
}
...
void mqttCallback(char* topic, byte* payload, unsigned int length){
  ...
  if(strcmp(topic, MQTT_TOPIC_GREEN)==0) setLedFromMessage(message, GREEN_LED_PIN, "GREEN");
  else if(strcmp(topic, MQTT_TOPIC_YELLOW)==0) setLedFromMessage(message, YELLOW_LED_PIN, "YELLOW");
  else if(strcmp(topic, MQTT_TOPIC_RED)==0) setLedFromMessage(message, RED_LED_PIN, "RED");
}
...
// Called by the e-Paper driver's BUSY wait loop.  E-paper refresh is
// synchronous, so keep MQTT callbacks alive while the panel is busy.
void serviceMqttDuringEpd(){
  if(WiFi.status()==WL_CONNECTED && mqttClient.connected()) mqttClient.loop();
}

void serviceNetwork(){
  if(WiFi.status()!=WL_CONNECTED){
    if(millis()-lastMqttAttempt>=15000UL){lastMqttAttempt=millis();startWiFi();}
    return;
  }
  if(!mqttClient.connected() && millis()-lastMqttAttempt>=MQTT_RETRY_INTERVAL_MS){
    lastMqttAttempt=millis();
    String clientId="esp32-epaper-"+String((uint32_t)ESP.getEfuseMac(),HEX)+String(random(0xffff),HEX);
    if(mqttClient.connect(clientId.c_str())){
      mqttClient.subscribe(MQTT_TOPIC_GREEN);
      mqttClient.subscribe(MQTT_TOPIC_YELLOW);
      mqttClient.subscribe(MQTT_TOPIC_RED);
      Serial.println("MQTT connected and control topics subscribed");
    }else Serial.printf("MQTT connect failed, state=%d\n",mqttClient.state());
  }
  mqttClient.loop();
  if(sensorDataPending) publishSensorData();
}

void publishSensorData(){
  if(!mqttClient.connected()) return;
  char payload[96];
  if(currentDhtOK) snprintf(payload,sizeof(payload),"{\"temp\":%d,\"hum\":%d,\"light\":%d}",currentTemperature,currentHumidity,currentLight);
  else snprintf(payload,sizeof(payload),"{\"temp\":null,\"hum\":null,\"light\":%d}",currentLight);
  if(mqttClient.publish(MQTT_DATA_TOPIC,payload,true)){ sensorDataPending=false; Serial.printf("MQTT TX [%s] %s\n",MQTT_DATA_TOPIC,payload); }
...
void readAndDisplay(bool fullRefresh){
  ...
  currentDhtOK=dhtOK; currentTemperature=temperature; currentHumidity=humidity; currentLight=lightPercent;
  ...
  Serial.println("e-Paper draw start");
  drawScreen(dhtOK,temperature,humidity,lightOK,lightPercent);
  Serial.println("e-Paper draw done, init start");
  if(epd.Init()!=0){Serial.println("e-Paper init failed");return;}
  Serial.println("e-Paper init done");
  if(fullRefresh){
    Serial.println("e-Paper full transfer start");
    epd.Display(blackBuffer,redBuffer);
    Serial.println("e-Paper full display updated");
  }else{
    // The V4 controller can leave BUSY asserted during the large rotated
    // partial window. Use the complete two-layer frame for reliable updates.
    // MQTT remains serviced by the BUSY wait hook in the driver.
    Serial.println("e-Paper periodic full transfer start");
    epd.Display(blackBuffer,redBuffer);
    Serial.println("e-Paper periodic full display updated");
  }
  epd.Sleep();
  Serial.println("e-Paper sleep done");
  publishSensorData();
}

void setup(){
  Serial.begin(115200); delay(300); analogReadResolution(12); pinMode(LIGHT_PIN,INPUT);
  ...
  Serial.println("40_epaper_mqtt"); startWiFi(); readAndDisplay(true); lastUpdate=millis();
}
void loop(){
  serviceNetwork();
  if(millis()-lastUpdate>=UPDATE_INTERVAL_MS){lastUpdate=millis();readAndDisplay(false);}
  delay(100);
}
